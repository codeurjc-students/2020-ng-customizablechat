...
export class ChatsGateway
  ...
  socketSave(client: Socket, username: string): void {
    this.logger.log(
      `Im going to save the client's socket! ${client.id}, ${username}`,
    );
  ...
  async sendMessagePrivate(client: Socket, message: CreateMessageDto) {
    try {
      ...
      const data = {
        message: await this.messagesService.saveMessage(message),
        isPrivate: true,
      };
      console.log(data);
      let user;
  ...
  async sendFileReceived(client: Socket, data: any) {
    this.logger.log(
      'Send message to participants that a new file is sent to the chat',
    );
    this.logger.log(data);
    const chat = await this.chatsService.findOneGroupById(data[2]);
    if (chat.isPrivate) {
      let user = await this.usersService.findOneByUsername(chat.participants);
      if (user.active == true)
