...
export class FilesController {
  ...
  async uploadFile(@UploadedFile() file, @Body() body) {
    ...
    const message = new CreateMessageDto(
      ...
      body.userName,
      body.chatId,
      file.size > 50000 ? 'compressed;' + file.mimetype : file.mimetype,
      file.buffer,
    );
    const data = await this.messagesService.saveMessage(message);
    return data;
  }
  ...
  async changeProfile(@UploadedFile() image, @Body() body) {
    ...
    const data = await this.usersService.changeProfile({
